...
class CheckSettings:
	def run(self):
		...
		profile_dir = userdata_path
		if not path_exists(profile_dir): make_directories(profile_dir)
		set_setting('version_number', addon().getAddonInfo('version'))
		wait_for_abort(0.5)
		make_settings_dict()
...
class FirstRunActions:
	def run(self):
		...
		addon_version, settings_version =  self.remove_alpha(addon().getAddonInfo('version')), self.remove_alpha(get_setting('version_number'))
