	...
	def _process():
		...
		for item in season_data:
			try:
				...
				if fanart_enabled:
					banner = season_art.get('seasonbanner_%s' % season_number, '') or show_banner
					thumb = season_art.get('seasonthumb_%s' % season_number, '') or show_fanart
				...
				set_properties({'watchedepisodes': string(watched), 'unwatchedepisodes': string(unwatched), 'totalepisodes': string(episode_count),
								'watchedprogress': string(progress), 'fen.playcount': string(playcount), 'fen.extras_params': extras_params, 'fen.options_params': options_params})
