...
class TVShows:
	...
	def build_tvshow_content(self, item_position, _id):
		try:
			...
			set_properties({'watchedepisodes': string(total_watched), 'unwatchedepisodes': string(total_unwatched), 'watchedprogress': string(progress),
							'totalepisodes': string(total_aired_eps), 'totalseasons': string(total_seasons), 'fen.sort_order': string(item_position),
							'fen.playcount': string(playcount), 'fen.extras_params': extras_params, 'fen.options_params': options_params})
