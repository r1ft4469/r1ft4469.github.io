...
class FontUtils:
	...
	def get_font_info(self):
		...
		try:
			root = ET.parse(self.skin_font_xml).getroot()[0]
			for item in root.iter('font'):
				try:
					try: name = item.find('name').text
					except: continue
					name_compare = name.lower()
					try: size = int(item.find('size').text)
					except: continue
					try: style = item.find('style').text.lower()
					except: style = ''
