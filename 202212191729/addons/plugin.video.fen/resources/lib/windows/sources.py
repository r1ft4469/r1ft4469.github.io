...
show_uncached_torrents, pack_check, run_plugin_str = ls(32088), ('true', 'show', 'season'), 'RunPlugin(%s)'
string = str
...
class SourceResults(BaseDialog):
	def __init__(self, *args, **kwargs):
		...
		self.window_type = self.window_format.split(' ')[0] if ' ' in self.window_format else self.window_format
		self.make_poster = self.window_type in ('list', 'medialist')
	...
	def make_items(self, filtered_list=None):
		...
		try:
			...
			if self.prescrape:
				...
				prescrape_listitem.setProperty('start_full_scrape', '[B]***%s***[/B]' % upper(start_scrape))
	...
	def set_poster(self):
		if self.current_poster:
			if self.window_type == 'list': self.getControl(200).setImage(self.current_poster)
			else: self.getControl(205).setImage(self.current_poster)
			...
			while not self.check_poster_cached(self.poster):
				...
				total_time += 1
				self.sleep(50)
			if self.window_type == 'list': self.getControl(200).setImage(self.poster)
			else: self.getControl(205).setImage(self.poster)
	...
	def filter_results(self):
		...
		if self.uncached_torrents: choices.append((show_uncached_torrents, 'show_uncached'))
